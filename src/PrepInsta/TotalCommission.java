...
public class TotalCommission {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Enter the total commission of parent(%): ");
        int per= sc.nextInt();

        System.out.println("Enter the total commission of the parent(%) if parent not have child: ");
        int perIf= sc.nextInt();

        System.out.println("Enter Y/y if parent has child otherwise N/n");
        ...
        if (ch == 'N' || ch == 'n') {
            ...
            int commissionIf=(perIf*totalAmount)/100;
            System.out.println(parent + ":"+commissionIf+" INR");
        } else {
            ...
            System.out.println("Enter the total commission of the child(%): ");
            int childPercent= sc.nextInt();

            int n = child.length;

            int totalParentCommission =((per*totalAmount)/100)*n;
            int totalChildCommission=(childPercent*totalAmount)/100;

            System.out.println("TOTAL MEMBERS:" + (n + 1));
            System.out.println("COMMISSION DETAILS");
            System.out.println(parent + ": " + totalParentCommission + " INR");

            for (String i : child) {
                System.out.println(i + ": "+totalChildCommission+" INR");
            }
