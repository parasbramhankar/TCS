...
public class RemainingMonkeys {
    public static void main(String[] args) {
        Scanner scanner =new Scanner(System.in);

//        System.out.println("Enter the total number of Monkeys: ");
        int n = scanner.nextInt();
//        System.out.println("Enter the number of banana and peanuts one monkey can eat: ");
...
//        System.out.println("Enter the number of peanuts: ");
        ...
        if(n<0 ||k<0 || m<0 ||p<0||j<0){
            System.out.println("Invalid input");
            System.exit(0);
        }

        int noOfMonkeyEatBanana=m/k;
        int remBanana=m%k;

        int noOfMonkeyEatPeanuts=p/j;
        int remPeanuts=p%j;

        int total=noOfMonkeyEatBanana+noOfMonkeyEatPeanuts;

        if(remPeanuts!=0 || remBanana!=0){
            total=total+1;
        }
        ...
        int remainingMonkey=n-total;

        System.out.println("The number of monkey left on the tree: "+remainingMonkey);
